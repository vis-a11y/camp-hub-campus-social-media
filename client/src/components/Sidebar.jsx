...
import { 
  ...
  User, LogOut, Zap, Layers, BarChart3
} from 'lucide-react';
...
const Sidebar = () => {
  ...
  return (
    <aside className="fixed left-0 top-0 h-screen w-[85px] xl:w-[280px] bg-white/80 dark:bg-[#020617]/80 backdrop-blur-3xl border-r border-slate-100 dark:border-white/5 z-50 transition-all duration-500 flex flex-col hidden lg:flex">
      ...
      <div className="flex items-center gap-3 p-8 mb-4 cursor-pointer" onClick={() => navigate('/dashboard')}>
      ...
      <div className="p-6 mt-auto border-t border-slate-100 dark:border-white/5">
        <div className="flex items-center gap-4">
           <div className="w-12 h-12 rounded-2xl overflow-hidden border-2 border-indigo-500/20">
              {user.profilePic ? <img src={user.profilePic} className="w-full h-full object-cover" /> : <div className="w-full h-full bg-indigo-500 flex items-center justify-center font-black text-white">{user.firstName?.[0]}</div>}
           </div>
           <div className="flex-1 hidden xl:block">
              <p className="text-[13px] font-black text-slate-900 dark:text-white truncate uppercase tracking-tighter">{user.firstName}</p>
              <button onClick={logout} className="text-[10px] font-bold text-slate-400 uppercase tracking-widest hover:text-rose-500 transition-colors">Terminate Sync</button>
           </div>
        </div>
      </div>
    </aside>
